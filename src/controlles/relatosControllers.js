...
// Configuração do multer (armazenar em memória)
const storage = multer.memoryStorage();
export const upload = multer({ storage: storage });

// Criar relato com suporte a foto/vídeo
export const relato = async (req, res) => {
    try {
        ...
        const relatoCriado = await relatosService.relato(nome, categoria, descricao, foto, video);
...
export const listarRelato = async (req, res) => {
    try {
        ...
        // Converter foto e vídeo para base64
        const relatosConvertidos = relatoListar.map(relato => {
            return {
                ...relato,
                foto: relato.foto ? relato.foto.toString("base64") : null,
                video: relato.video ? relato.video.toString("base64") : null
            };
        });

        res.status(200).json({ messagem: "Aqui estão todos os relatos!", data: relatosConvertidos });
    } catch (error) {
        console.log(error);
        res.status(404).json({ message: "Não foi possível encontrar os relatos, tente novamente" });
    }
