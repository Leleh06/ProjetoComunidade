...
export const logarONG = async (req, res) => {
    try {
        ...
        const ongLogada = await ongService.logarONG(email, senha);

        if (ongLogada && ongLogada.length > 0) {
            res.status(200).json({ message: "Ong logada com sucesso!", data: ongLogada[0] });
        } else {
